...
function ExportScreen() {
  const { patientId } = useParams({
    from: '/_authed/patients/$patientId/export',
  })
        ...
        <div className="border-2 border-foreground/80 bg-white">
          ...
          <div className="p-4 sm:p-8 border-b-2 border-foreground/80">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              ...
              <div>
                <p className="font-mono text-xl sm:text-2xl font-black text-amber-600">
                  {lateCount}
                </p>
                <p className="text-xs text-muted-foreground">Late</p>
              </div>
              <div>
                <p className="font-mono text-xl sm:text-2xl font-black text-red-600">
                  {missedCount}
                </p>
            ...
            {missedCount > 0 && (
              <div className="mt-4 p-3 bg-red-50 border-2 border-red-200">
                <p className="text-xs sm:text-sm font-bold text-red-800">
                  {missedCount} missed dose{missedCount !== 1 ? 's' : ''} during
                  this period
                </p>
                  ...
                  {filteredLogs
                    ...
                    .map((l) => {
                      ...
                      return (
                        <li
                          key={l._id}
                          className="font-mono break-all sm:break-normal"
                        >
                          · {med?.name} — {formatDate(l.takenAt)}
                          {l.notes ? ` (${l.notes})` : ''}
                        </li>
                      )
                    })}
              ...
              <Table className="min-w-[640px]">
                ...
                <TableBody>
                  {filteredLogs.map((log) => {
                    ...
                    const scheduledHour = med.scheduledTimes.reduce(
                      (closest, h) =>
                    ...
                    return (
                      <TableRow key={log._id} className="font-mono text-sm">
                        ...
                        <TableCell>
                          {log.missed ? '—' : formatTimestamp(log.takenAt)}
                        </TableCell>
                        <TableCell>
